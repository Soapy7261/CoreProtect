...
List<SignResult> signLookup(Location location, int time, int radius)
List<SignResult> signLookup(String user, int time)
List<SignResult> signLookup(String user, int time, int radius, Location radius_location)
```
...
CoreProtectAction
```
- `chatLookup`, `commandLookup`, `signLookup`, and `sessionLookup` now support `LookupOptions` overloads for shared user, time, location, radius, and limit options.
...
`BlockResult`, `ContainerResult`, `MessageResult`, `SessionResult`, `SignResult`, and `ParseResult` implement `CoreProtectResult`, which provides shared access to player, timestamp, coordinates, world, and action data.
...
#### `signLookup(Location location, int time, int radius)`

This will perform a sign text lookup around a location, similar to running `/co lookup a:sign r:<radius>`.
...
#### `signLookup(String user, int time, int radius, Location radius_location)`
...
CoreProtectAPI CoreProtect = getCoreProtect();
if (CoreProtect != null){ // Ensure we have access to the API
  List<SignResult> lookup = CoreProtect.signLookup("Notch", 60, 5, location);
  if (lookup != null){
    for (SignResult result : lookup){
      int x = result.getX();
      int y = result.getY();
