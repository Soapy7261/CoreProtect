...
CoreProtectAction
```
- `chatLookup`, `commandLookup`, and `signLookup` now support `LookupOptions` overloads for shared user, time, location, radius, and limit options.
...
`ContainerResult`, `MessageResult`, `SignResult`, and `ParseResult` implement `CoreProtectResult`, which provides shared access to player, timestamp, coordinates, world, and action data.
